...
Router.route('/posts/:tag?', {
    ...
    onAfterAction: function() {
        ...
        if (!_tag) {
            // console.log("no tag");
            Session.set('selectedTag', 'List all');
        } else {
            // console.log(_tag);
            Session.set('selectedTag', _tag.slice(0, 10));
        }
    }
...
Router.route('/submit', {
    name: 'postSubmit',
    onAfterAction: function() {
        Session.set('selectedTag', 'Submit');
    }
...
// Router.onAfterAction(setTagList, {
//     only: ['postSubmit', 'postEdit']
// });
